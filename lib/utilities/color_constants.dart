import 'package:flutter/material.dart';

class DarkColorConstants {
  DarkColorConstants._();

  static final primaryColor = Color.fromARGB(255, 4, 5, 29);
  static final secondaryColor_1 = const Color.fromARGB(255, 24, 128, 224);
  static final secondaryColor_2 = const Color.fromARGB(255, 18, 16, 41);
  static final tertiaryColor = const Color.fromARGB(255, 255, 255, 255);
  static final gradientColor1 = const Color.fromARGB(224, 7, 139, 126);
  static final gradientColor_2 = const Color.fromARGB(255, 112, 153, 232);
}

class LightColorConstants {
  LightColorConstants._();
  static final primaryColor = const Color.fromARGB(255, 141, 176, 244);
  static final secondaryColor_1 = const Color.fromARGB(255, 86, 130, 215);
  static final secondaryColor_2 = Color.fromARGB(255, 28, 68, 148);
  static final tertiaryColor = const Color.fromARGB(255, 0, 0, 0);
  static final gradientColor_1 = const Color.fromARGB(224, 7, 139, 126);
  static final gradientColor_2 = const Color.fromARGB(255, 112, 153, 232);
  static final seedColor = const Color.fromARGB(225, 28, 68, 146);
}
